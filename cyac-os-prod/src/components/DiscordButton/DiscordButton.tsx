...
const DiscordButton: React.FC<DiscordButtonProps> = ({ className }) => {
    ...
    // Discord invite link
    const discordInvite = 'https://discord.gg/ukDMHn6YT9';

    // Random glitch effect
    useEffect(() => {
        if (hovering) {
            const glitchInterval = setInterval(() => {
                if (Math.random() < 0.3) { // 30% chance of glitch when hovering
                    setGlitching(true);
                    setTimeout(() => {
                        setGlitching(false);
                    }, 150);
    ...
    // Handle click - open discord link
    const handleClick = () => {
    ...
    return (
        <div
            className={`${styles.discordButtonContainer} ${glitching ? styles.glitching : ''} ${hovering ? styles.hovering : ''} ${className || ''}`}
        ...
        >
            <div className={styles.discordButton}>
                ...
                <button className={styles.copyButton} onClick={copyInvite}>
                    {copied ? 'COPIED!' : 'COPY'}
                </button>
            </div>
