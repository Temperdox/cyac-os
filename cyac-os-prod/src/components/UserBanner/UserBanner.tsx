...
const UserBanner: React.FC<UserBannerProps> = ({ onLogout }) => {
    ...
    // Generate banner URL if available
    const getBannerUrl = () => {
        if (!user || !user.banner) {
            return null;
        }
    ...
    // Get user level (could be based on your application's logic)
    const getUserLevel = () => {
        if (!user) return 1;

        // Example: determine level based on some criteria
        // This could be modified based on your actual level determination logic
        return 1; // Default level
    };

    const bannerStyle = getBannerUrl()
        ? { backgroundImage: `url(${getBannerUrl()})` }
        : {};

    return (
        <div className={styles.userBanner}>
            <div className={styles.bannerBackground} style={bannerStyle}>
            ...
            <div className={styles.userContent}>
                {isAuthenticated ? (
                    ...
                    <div className={styles.notAuthenticated}>
                        <button className={styles.loginButton} onClick={() => DiscordAuthService.login()}>
                            <div className={styles.discordLogo}>
                                <svg width="20" height="20" viewBox="0 0 71 55" fill="#33ff33" xmlns="http://www.w3.org/2000/svg">
